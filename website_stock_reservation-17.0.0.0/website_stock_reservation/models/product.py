...
class ProductTemplate(models.Model):
    ...
    reserve_products = fields.Boolean('Reserve Products')
    
    _defaults = {
        'reserve_products' : False
    }
...
class Website(models.Model):
    ...
    def get_reserve_products(self):   
        reserve_ids=self.env['product.template'].search([('reserve_products','=',True),("website_published", "=", True)])
        return reserve_ids  

    def get_res_config_values(self):   
        res_ids=self.env['res.config.settings'].sudo().search([])
        return res_ids
...
class Website(models.Model):
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        if not sale_order_id and request.session.get('from_reserve_pro') == 'yes' and not self.env.user._is_public():
            sale_order_id = None
            flag = True
        ...
        if self.env['product.pricelist'].browse(force_pricelist).exists():
            ...
            pricelist_id = request.session.get('website_sale_current_pl') or self.pricelist_id.id
